frappe.ui.form.on('Design', {
  ...
  fnComputeCustomLosses(frm, iLossesSetting) {
      const LOSSES_SETTINGS = iLossesSetting.filter(item => item.transformer_type === frm.doc.transformer_type);
      ...
      const L_INDEX = LOSSES_SETTINGS.findIndex(x => parseInt(x.rating, 10) > L_RATING_INT);
    ...
    * @params frm 
    * @params iSetDefaultValue - Flag to determine if default values should be set.
    ...
    */
  fnGetStandardLosses(frm, iSetDefaultValue, iFieldForValidation, iSettingsField) {
      ...
      frappe.model.with_doc(ldDoctype, ldDoctype, function() {
          ...
          if (!lRatingLosses) {
              ...
              lRatingLosses = frm.events.fnComputeCustomLosses(frm, LA_VALUES[0].losses_setting);
          }
          ...
          if(iSetDefaultValue) {
           ...
           if((frm.doc[iFieldForValidation] > lRatingLosses[iSettingsField]) 
