    ...
    if (frm.is_new()) {
      ...
      frappe.model.with_doc(lDoctype, lDoctype, function () {
        ...
        frm.doc.max_average_temperature_per_year =
          laValues[0].max_average_temperature_per_year;
        frm.doc.max_average_temperature_per_month =
          laValues[0].max_average_temperature_per_month;
  ...
  fnCalculateTemperatureRise(frm, on_field) {
    ...
    frappe.model.with_doc(lDoctype, lDoctype, function () {
      ...
      var lTemperatureDelta = Math.max(
        frm.doc[on_field] - laValues[0][on_field],
        0
      );

      var ldIpProtectionRating = laValues[0].ip_protection_setting.find(
        (x) => x.rating === frm.doc.rating
      );
      //Find the next higher rating for custom rating
      // (ie if standard rating is not found)
      if (!ldIpProtectionRating) {
        ldIpProtectionRating = laValues[0].ip_protection_setting.find(
          (x) => x.rating > frm.doc.rating
        );
      }
      const L_IPPROTECTION_TEMPERATURE =
        ldIpProtectionRating[frm.doc.ip_protection.toLowerCase()];
      //Temperature rise datasheet will be used in datasheets.
      // This is the data that will be shared to the customer
      //in the transformer technical datasheet.
      frm.doc.temperature_rise_datasheet =
        frm.doc.temperature_rise -
        lTemperatureDelta * laValues[0].temperature_rise_ratio;
      //Temperature rise will be used for Gitra Calculation 
      // which includes IP protection factors as well
      const L_TEMPERATURE_DELTAWITHIP =
        L_IPPROTECTION_TEMPERATURE - lTemperatureDelta;
      frm.doc.temperature_rise_gitra =
        frm.doc.temperature_rise +
        L_TEMPERATURE_DELTAWITHIP * laValues[0].temperature_rise_ratio;
      //Harmonize the average temperatures. 
      // For eg max_average_temperature_per_year is increased by 3 degree add 3 degree
      //to max_average_temperature_per_year and ambient_max_temperature
      frm.doc.max_average_temperature_per_month =
        laValues[0].max_average_temperature_per_month + lTemperatureDelta;
      frm.doc.max_average_temperature_per_year =
        laValues[0].max_average_temperature_per_year + lTemperatureDelta;
      frm.doc.ambient_max_temperature =
        laValues[0].ambient_max_temperature + lTemperatureDelta;
