frappe.ui.form.on("Design", {
  ...
  function fncheckRecalculateDesign(frm) {
    ...
    const timeDiff = Math.abs(today - lastCalculateDate);
    const dayDiff = Math.ceil(timeDiff / (1000 * 3600 * 24));
    if (dayDiff >= refreshDate && frm.doc.status == 'Calculation Received') {
        fnShowButtonGroup(frm, 'Recalculate Design', fnRecalculate);
    } else {
        fnUpdateButtonGroup(frm);
    }
...
function fnRecalculate(frm){
    const DATEPART = frm.doc.upstream_file.split('_')[0];  // Extract the part before the first underscore
    ...
    // Combine the date and time with an underscore
    const formattedDateTime = `_${formattedDate}_${formattedTime}`;

    frm.set_value('upstream_file', DATEPART + formattedDateTime);
