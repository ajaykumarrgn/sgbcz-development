...
validate(frm){
    ...
    if(!frm.doc.description || frm.doc.description=='<div class="ql-editor read-mode"><p><br></p></div>' || frm.is_new()){
        if (frm.doc.variant_of ) { 
            ...
            // Previously, "IP Rating" was the rating attribute, but it has now been changed to "Power (kVA)"
            //lIndex = frm.doc.attributes.findIndex(element => element.attribute=="IP Rating"); //<<ISS-2024-00004

            lIndex = frm.doc.attributes.findIndex(element => element.attribute == "Power (kVA)"); //<<ISS-2024-00004
            if (lIndex >= 0) {
            ...
            //Previously, "eNCLOSURE" was the attribute, but it has now been changed to "Enclosure IP Rating"
            ...
            //>>ISS-2024-00004
            //Cooling method not include in the item descrtion for the Enclosure template 
        ...
        //Fill item description as 'Item Group specification:' if nothing is filled from Above
        if (!lItemDescription) {
