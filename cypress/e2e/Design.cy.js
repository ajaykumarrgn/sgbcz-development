describe('qsgbcz calculation test ', () => {
  beforeEach(() => {
  ...
  it('Adding the Design', () => {
    ...
    cy.get('@testdata').then((testdata) => {
      ...
      //cy.get('div[data-fieldname="status"] > .form-group > .control-input-wrapper > .control-input > .input-with-feedback').should('have.value',testdata[11].value);
         
    });
  });

  //Run script to update the status

  it('Run Node Script After Design created', () => {
    // Change directory, install dependencies, and execute the Node.js scripts
    cy.exec('cd "gitraTesting" && npm i && node getDesign.js && node updateDesign.js', { failOnNonZeroExit: false }).wait(10000)
      .then((result) => {
