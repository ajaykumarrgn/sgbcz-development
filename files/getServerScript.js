...
// Function to create the "serverScript" folder if it doesn't exist
function ensureServerScriptFolderExists() {
  const serverScriptFolderPath = path.join(current_path,'..', 'serverScript');
  if (!fs.existsSync(serverScriptFolderPath)) {
    fs.mkdirSync(serverScriptFolderPath, { recursive: true });
    console.log('Created "serverScript" folder.');
  }
}

// Ensure "serverScript" folder exists
ensureServerScriptFolderExists();

const baseUrl = getEndPointForDoctype("Server Script");

fetch(`${baseUrl}?fields=["*"]&filters={\"disabled\":0}&limit_page_length=0`, requestOptions)
  .then(response => {
    if (!response.ok) {
  ...
  .then(data => {
    if (data && data.data && Array.isArray(data.data)) {
      data.data.forEach(documentDetails => {
        ...
        const folderPath = path.join(current_path,'..', 'serverScript', folderName);
