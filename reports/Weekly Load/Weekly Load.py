...
def fn_get_columns(i_filters):
    ...
    la_columns = [{"fieldname": "power", "label": _("power"), "fieldtype": "Data", "width": 100},
    ...
    for wk in range(int(l_from_week), int(l_to_week)):
        ld_column = dict({"fieldname": str(wk), "label" : _(str(wk)), "fieldtype": "Int", "width": 50 })
...
#  Begin of sub functions
    ...
    def fn_get_planning_row_dict(i_from_date, i_to_date):
        ...
        ld_planning_row = {str(week): 0 for week in range(int(l_from_week), (int(l_to_week)+1))}
    ...
    def fn_get_schedule_lines(i_filters):
        ...
        ld_schedule_lines = frappe.db.sql("""select pos, planned_production_end_date, parent
    ...
    def fn_get_delivery_items(ia_delivery_notes, ia_unique_pos):
        ...
        ld_et_delivery_items = sorted(ld_et_delivery_items, key=lambda x: (x['parent'], x['pos']) )
    ...
    def fn_add_parallel_to_rating(ia_transformers_rating, ia_unique_parallel):
        ...
        for item in ( item for item in ia_transformers_rating if (item['parent'] in ia_unique_parallel)):
    ...
    def fn_get_weekly_planned_capacity_row(i_planned_capacity, i_planning_row, i_parallel_row):
        ...
        for i_weekly_capacity in i_planned_capacity['planned_capacity_item']:
            ...
            # Calculate the number of transformers to be reduced for every 3 parallel coils per week
                ...
                # Get the planned weekly capacity and reduce it by the parallel factor from above step
            i_planning_row[l_week_number] = max(0, int(i_weekly_capacity['qty']) - transformers_to_be_reduced)
    ...
    def fn_get_final_data_records(i_filters, ia_unique_rating, ia_transformers_rating, i_delivery_note_items, i_schedule_lines, id_planned_capacity):
        ...
        ld_planning_row_dict = fn_get_planning_row_dict(i_filters.from_date, i_filters.to_date)
        
        # Create dict for parallel row
        ld_parallel_row = dict(ld_planning_row_dict)
        ld_parallel_row['power'] = 'Parallel'
        ld_parallel_row['weekly_capacity'] = id_planned_capacity.parallel_weekly_capacity
        ...
        for i_rating in ia_unique_rating:
            ...
            for la_item in (la_item for la_item in ia_transformers_rating if la_item['attribute_value'] == i_rating):
                for i_delivery_item in (i_delivery_item for i_delivery_item in i_delivery_note_items if i_delivery_item['item_code'] == la_item['parent']):
                    for i_schedule_line in i_schedule_lines:
                        if (i_schedule_line['parent'] == i_delivery_item['parent']) and i_schedule_line['pos'] == i_delivery_item['pos']:
                            l_pl_wk = fn_get_calander_week(i_schedule_line['planned_production_end_date'])
                            #frappe.msgprint("planning week" + schedule_line['parent'] + str(l_pl_wk))
                            ld_planning_row[l_pl_wk] = ld_planning_row[l_pl_wk] + 1
                            
                            ld_parallel_row = fn_get_sum_of_parallel(ld_planning_row, l_pl_wk, ld_parallel_row)
                            ld_total_row = fn_get_total(ld_planning_row, l_pl_wk, ld_total_row)
            
            # Add the weekly capacity for the respective rating 
            for ld_rating_capacity in (ld_weekly_capacity for ld_weekly_capacity in id_planned_capacity['rating_weekly_capacity'] if ld_weekly_capacity['rating'] == i_rating):
                ld_planning_row['weekly_capacity'] = ld_rating_capacity['weekly_capacity']
        ...
        la_e_final_data.append(fn_get_weekly_planned_capacity_row(id_planned_capacity, dict(ld_planning_row_dict), ld_parallel_row) )
    ...
    def fn_get_planned_capacity(i_filters):
        l_calander_year = frappe.utils.formatdate(i_filters.from_date, format_string="YYYY")
        ld_et_planned_capacity = frappe.get_doc('Planned Capacity', l_calander_year).as_dict()
...
#   End of sub functions
    ...
    la_item_rating = fn_add_parallel_to_rating(la_transformers_rating, la_unique_parallel)
    ...
    # Final output
    la_final_data = fn_get_final_data_records(i_filters, la_unique_rating, la_transformers_rating, la_delivery_note_items, la_schedule_lines, la_planned_capacity)
