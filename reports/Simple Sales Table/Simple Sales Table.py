...
def get_columns(filters):
    ...
    la_columns = [
        ...
        {"fieldname": "power_lv1", "label": _(
            "Rating LV1"), "fieldtype": "Data", "width": 80},
        {"fieldname": "power_lv2", "label": _(
            "Rating LV2"), "fieldtype": "Data", "width": 80},
        ...
        {"fieldname": "lv1", "label": _(
            "LV 1 Volt"), "fieldtype": "Data", "width": 70},
        {"fieldname": "lv_2", "label": _(
            "LV 2 Volt"), "fieldtype": "Data", "width": 70},
        {"fieldname": "vector_group", "label": _(
            "Vector Group"), "fieldtype": "Data", "width": 70},
        {"fieldname": "vector_group_lv1", "label": _(
            "Vector Group LV1"), "fieldtype": "Data", "width": 70},
        {"fieldname": "vector_group_lv2", "label": _(
            "Vector Group LV2"), "fieldtype": "Data", "width": 70},
        {"fieldname": "uk", "label": _(
            "UK %"), "fieldtype": "Data", "width": 50},
        {"fieldname": "uk_lv1", "label": _(
            "UK LV 1 %"), "fieldtype": "Data", "width": 50},
        {"fieldname": "uk_lv2", "label": _(
            "UK LV 2 %"), "fieldtype": "Data", "width": 50},
...
#   Few accessories need prefix - refer to individual accessories section for the prefix logic
    ...
    def map_accessories(item, po_item_row, ima_items):
        ...
        if item['accessories_specification']:
            ...
            l_index = binary_search_leftmost(ima_items, 'item_code', item.item_code)
        ...
        # Initialize accessory string
        accessory_str = po_item_row.get('accessories', '')
    
        # Append accessory specifications based on item group
        if ld_item_master["item_group"] == 'Antivibration Pads':
            accessory_str = accessory_str + f"Antivibration Pads: {accessories_specification}, "
        elif ld_item_master["item_group"] == 'Enclosure':
            accessory_str = accessory_str + f"Enclosure: {accessories_specification}, "
        elif ld_item_master["item_group"] == 'Ball Points':
            accessory_str = accessory_str + f"Ball Points: {accessories_specification}, "
        elif ld_item_master["item_group"] == 'CUPAL':
            accessory_str = accessory_str + f"CUPAL: {accessories_specification}, "
        elif ld_item_master["item_group"] == 'Control Unit':
            if accessories_specification not in po_item_row.get('control_unit', ''):
                accessory_str = accessory_str + f"Control Unit: {accessories_specification}, "
        elif ld_item_master["item_group"] == 'Services':
            accessory_str = accessory_str + f"Test Lab: {item.item_code}, "
            
        elif ld_item_master["item_group"] == 'Others 2':
            accessory_str = accessory_str + f"Others 2: {item.item_code}, "
           
        elif ld_item_master["item_group"] == 'Others':
            if item.item_code.startswith("Fan"):
                accessory_str = accessory_str + "Fan: Yes, "
            elif item.item_code.startswith("Forklift"):
                accessory_str = accessory_str + "Forklift: Yes, "
            elif item.item_code.startswith("Earthing switch"):
                accessory_str = accessory_str + f"Earthing Switch: {accessories_specification}, "
            elif item.item_code.startswith("Surge arrester"):
                accessory_str = accessory_str + f"Surge Arrester: {accessories_specification}, "
            elif item.item_code.startswith("Extended"):
                accessory_str = accessory_str + f"Extended Busbars: {accessories_specification}, "
    
        # Remove the trailing comma and space
        accessory_str = accessory_str.rstrip(',')
...
#   first one is the entire sales order details and second one is the unique sales order numbers
        ...
        for ld_item in ld_items_rev_sorted:
            ...
            for ld_setid in range(0, int(ld_item.qty)):
                if ld_setid < len(l_order_values):
                    ...
                    l_order_values.append(ld_item.base_rate) 
    ...
    #       Only get the Main items. Ignore accessories and services
            if (ld_item.pos % 10 == 0):
                ...
                po_item_row['price_gts'] = ld_item.base_rate 
