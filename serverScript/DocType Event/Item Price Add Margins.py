...
if doc.price_list == "Standard Selling":
    ...
    if l_item.item_group == "DTTHZ2N" and l_item.design:
        ...
        for ld_price_list in ld_price_lists:
            ...
            if ld_price_list.selling == 1 and (
                ld_price_list.ebitda > 0
                or ld_price_list.provision > 0
                or ld_price_list.transport > 0
            ):
                # Get the existing item price for the price list
                l_item_price_doc = fn_get_item_price(doc.item_code, ld_price_list.name)

                # >>US-2024-0167:Uncomment this block of code for
                # updating price list through the Design item
                # If an Item price already found for the price list
                if l_item_price_doc:
                    # get the item price document
                    ld_price_list_item_price = frappe.get_doc(
                        "Item Price", l_item_price_doc.name
                    )
                    # Modify the price with margins from the standard selling price
                    ld_price_list_item_price.price_list_rate = (
                        doc.price_list_rate
                        + doc.price_list_rate * ld_price_list.ebitda / 100
                    )
                    # add privsion
                    ld_price_list_item_price.price_list_rate = (
                        ld_price_list_item_price.price_list_rate
                        + ld_price_list_item_price.price_list_rate
                        * ld_price_list.provision
                        / 100
                    )
                    # add transport
                    ld_price_list_item_price.price_list_rate = (
                        ld_price_list_item_price.price_list_rate
                        + ld_price_list.transport
                    )
                    # round to nearest 10
                    ld_price_list_item_price.price_list_rate = (
                        round(ld_price_list_item_price.price_list_rate / 10) * 10
                    )
                    # Save the document
                ...
                if not l_item_price_doc:
                    ...
                    # # Modify the price with margins from the standard selling price
                    # ebitda_value is calculated as material_cost*ebitda/(100-ebitda)
                    # eg 21140*25/(100-25) = 7046.7
                    # and price with ebitda is calculated
                    # as ebitda_value + doc.total_cost
                    ld_target_item_price.price_list_rate = doc.price_list_rate + (
                        ld_price_list.ebitda
                        * doc.price_list_rate
                        / (100 - ld_price_list.ebitda)
                    )
                    # add transport
