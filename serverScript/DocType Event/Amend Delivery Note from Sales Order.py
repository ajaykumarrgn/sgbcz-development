...
def fn_amend_cancelled_delivery_note(i_cancelled_delivery_note, i_doc):
    ...
    def fn_sort_schedule_lines_by_date(i_target_lines):
        ...
        la_target_lines = sorted(
            la_target_line_with_date, key=lambda d: d.planned_production_end_date
        )
    ...
    # should be removed
    def fn_make_target_lines_per_so_item(i_d_item, i_a_source_lines):
        # Initialize the serial no array
        la_serial_numbers = []

        la_target_lines = i_a_source_lines

        # if serial no is present in the sales order item
        if i_d_item.serial_no:
            # then get the individaul serial number to another array
            la_serial_numbers = i_d_item.splitlines()

        # Check If sales order quantity is revised with more quantity
        if len(i_a_source_lines) < int(i_d_item.qty):
            ...
            for i in range(len(i_a_source_lines), int(i_d_item.qty)):
                ld_schedule_row = {}
                ld_schedule_row["pos"] = i_d_item.pos
                ld_schedule_row["item_code"] = i_d_item.item_code
                ld_schedule_row["status"] = "Purchasing"
                ld_schedule_row["delivery_date"] = (i_d_item.delivery_date,)
                ld_schedule_row["oa_confirmed_date"] = i_d_item.delivery_date
                la_target_lines.append(ld_schedule_row)

        # chedk if sales order quantity is revised with less quantity
        elif len(i_a_source_lines) > int(i_d_item.qty):
            ...
            la_target_lines = la_target_lines[: int(i_d_item.qty)]
        return la_target_lines

    def fn_get_source_schedule_lines_for_the_item(i_d_item, i_d_source_schedule_lines):
        la_item_schedule_lines = [
            d
            for d in i_d_source_schedule_lines
            if d.get("item_code") == i_d_item.item_code 
                and d.get("pos") == i_d_item.pos #<< ISS-2024-00093
        ]
    ...
    for i_d_item in (i_d_item for i_d_item in i_doc.items if i_d_item.pos % 10 == 0):

        la_item_source_lines = fn_get_source_schedule_lines_for_the_item(
            i_d_item, ld_cancelled_del_doc.delivery_schedule
        )
        la_item_target_lines = fn_make_target_lines_per_so_item(
            i_d_item, la_item_source_lines
        )
...
if doc.amended_from:
    ...
    ld_cancelled_delivery_note = frappe.get_all(
        "Delivery Note Item",
        fields=["parent"],
    ...
    if ld_cancelled_delivery_note:
        # Amend the cancelled delivery note
        fn_amend_cancelled_delivery_note(ld_cancelled_delivery_note[0].parent, doc)
