...
def compute_final_price(doc, price_list, item_price_doc):
    ...
    item_price_doc.price_list_rate = doc.total_cost  + (price_list.ebitda * doc.total_cost/(100-price_list.ebitda))
    ...
    if price_list.provision:
        # add provsion
        item_price_doc.price_list_rate = item_price_doc.price_list_rate  + (price_list.provision * item_price_doc.price_list_rate/(100-price_list.provision))
        # round to nearest 10
    item_price_doc.price_list_rate = ((item_price_doc.price_list_rate + add_nine)//10)*10 
    ...
    if doc.material_cost:
        ...
        for price_list in doc.item_price_override:
            ...
            if not item_price_doc:
                ...
                item_price_doc = frappe.get_doc('Item Price', item_price_doc.name)  # Fetch the existing document
                item_price_doc = compute_final_price(doc, price_list, item_price_doc)
