...
def fn_find_dict_with_keys(doc, i_dict_array):
    ...
    l_predicate = lambda x: int(x.get('rating', 0)) >= int(doc.get('rating', 0)) and int(x.get('hv', 0)) >= int(doc.get(l_predicatehv_field))
...
if not doc.labour and doc.direct_material_cost:
    ...
    #doc.total_cost = round((doc.cost_of_goods + doc.sales_overhead + doc.administrative_overhead)/10)*10
