...
def find_row_by_key_value(ia_dict_array, i_key, i_value):
...
def find_dict_with_keys(id_dict_row, ia_dict_array):
    ld_predicate = lambda x: int(x.get('rating')) >= int(id_dict_row.get('rating')) and int(x.get('hv',0)) >= int(id_dict_row.get('hv_rated_voltage'))
...
if doc.material_cost: 
    ...
    #<<ISS-2024-00057   
        ld_rating_row = find_row_by_key_value(doc.attributes, 'attribute', 'Power (kVA)')
        ld_hv_row = find_row_by_key_value(doc.attributes, 'attribute', 'HV (kV)')
        ld_attributes = {
            #'rating': ld_rating_row.attribute_value,
            #'hv_rated_voltage': ld_hv_row.attribute_value,
            'rating': ld_rating_row.attribute_value, 
            'hv_rated_voltage': int(float(ld_hv_row.attribute_value.replace(',', '.')) * 1000),
        }
        ...
        ld_design_production_hours = find_dict_with_keys(ld_attributes, ld_gitra_settings.labour_hours)
        ...
        l_production_overhead =  ld_design_production_hours.hours * ld_gitra_settings.production_rate
    ...
    l_administrative_overhead = l_cost_of_goods * ld_gitra_settings.administrative_overhead/100
