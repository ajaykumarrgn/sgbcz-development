def validate_serial_number_duplicate(serial_number, schedule, doc):
    if serial_number:
        duplicate = frappe.db.exists(
            "Delivery Schedule", {"serial_number": serial_number, "docstatus": 1}
        )
        if duplicate:
            # Check if the serial number is present in other Delivery Note schedules
            count = frappe.db.count(
                "Delivery Schedule",
                {"serial_number": serial_number, "parent": ["!=", schedule.parent]},
            )
    ...
    count = sum(
        ...
        if schedule_item.serial_number == serial_number
    )
