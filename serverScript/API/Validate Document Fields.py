...
def fn_add_error_message(ia_response, code, msg, field_name):
    ...
    ia_response.append(
        {"code": code, "msg": _(f"{msg} {field_label}"), "msgtype": "Error"}
    )
...
def fn_validate_field(i_field_validation_rules, i_value, ia_response):
    ...
    if i_value:
        ...
        # Validate separator
        fn_validate_separator(
            i_field_validation_rules, i_value, ia_response, field_label
        )

        # Validate field length and type based on whether separator exists
        if i_field_validation_rules.separator:
            fn_validate_with_separator(
                i_field_validation_rules, i_value, ia_response, field_label
            )
        else:
...
def fn_validate_separator(i_field_validation_rules, i_value, ia_response, field_label):
    if (
        ...
        fn_add_error_message(
            ia_response, 301, "Separator Error:", i_field_validation_rules.field_name
        )


# Validate prefix length and type (for fields with separators)
def fn_validate_with_separator(
    i_field_validation_rules, i_value, ia_response, field_label
):
    ...
    if len(la_split) > 1:
        ...
        fn_validate_suffix_length_and_type(
            i_field_validation_rules, suffix_value, ia_response, field_label
        )
...
def fn_validate_part_length(i_length, part_value, ia_response, part_name, field_label):
    if len(part_value) != i_length:
        fn_add_error_message(
            ia_response, 301, f"{part_name} Length Error:", field_label
        )
...
def fn_validate_part_type(i_type, part_value, ia_response, part_name, field_label):
    if i_type == "int" and not part_value.isdigit():
        fn_add_error_message(
            ia_response,
            301,
            f"{part_name} Type Error:",
            field_label + " must be an integer.",
        )
...
def fn_validate_suffix_length_and_type(
    ...
    if i_field_validation_rules.part_2_length != 0:
        ...
        if (
            ...
            and len(suffix_value) != i_field_validation_rules.part_2_length
        ):
...
def fn_validate_suffix_condition(
    ...
    if suffix_value.isdigit():
        ...
        if suffix_number % i_field_validation_rules.part_2_condition != 0:
            fn_add_error_message(
                ia_response, 301, "Invalid SAP Reference Number:", field_label
            )
...
for l_field_validation_rules in la_field_validation_rules:
    if l_field_validation_rules.is_child:
        for l_child in lo_doc.get(l_field_validation_rules.child_field_name) or []:
            ...
            la_response.extend(
                fn_validate_document_field(l_field_validation_rules, l_value)
            )
    else:
...
if not la_response:
    la_response.append(
        {"code": 200, "msg": "Validation Successful", "msgtype": "Success"}
    )
